...
This project models a synthetic SaaS business to answer a common operational and leadership question:
...
The goal of the project is not to build a large analytics platform, but to demonstrate clear and correct analytics engineering practices for subscription and churn reporting.

The project is designed to be readable in a few minutes by a data lead.
...
These represent operational system extracts.
...
This guarantees that months with zero revenue or no activity are still represented in the fact table.
...
This model is designed to be directly consumable by BI tools and ad-hoc analysis.
...
This allows historical analysis of how account subscription states evolve over time without re-deriving past states.
...
- time series reporting remains consistent
...
- clear separation between point-in-time state and monthly performance
...
The project includes example analysis queries for:
...
These queries demonstrate how the mart layer is intended to be consumed.
...
All models, tests, and snapshots should complete successfully with `dbtf build`.
